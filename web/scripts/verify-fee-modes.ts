...
async function main() {
  ...
  for (const mode of ["keep", "holders", "burn", "project"] as Mode[]) {
    ...
    const creatorUsdc = (await bal(USDC, account.address)) - creatorUsdc0;
    ...
    if (mode === "holders") {
      ...
      check("creator was not paid instead", creatorUsdc === 0n);
      ...
      check("and claim it, in USDC", (await bal(USDC, account.address)) - before === pending);
    }
    if (mode === "burn") {
    ...
    if (mode === "project") {
      ...
      check("the creator earned nothing from it", creatorUsdc === 0n && creatorTok === 0n);
    }
  }
